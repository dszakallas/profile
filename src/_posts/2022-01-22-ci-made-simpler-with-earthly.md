...
Differences between the CI and the developer's local environment can lead to unreprodicible builds, and the "works on my machine" effect. Modern CI runners can utilize containers, however locally running these steps can be a chore. You can use CI specific helpers (e.g there's a tool called [act](https://github.com/nektos/act) which can be used to simulate GitHub Action Workflows locally), however using a platform specific solution feels a desperate attempt at curbing complexity. The puritan engineer would rather opt for a thin shell (or `make`, `npm` script, etc.) wrapper around each CI job, so that it consist of a single script command invocation both in the CI config file as well as running locally in container with `docker run`. However this approach is oblivious to some important aspects of your CI build. First, although usually not a problem when running locally, caching needs to be taken into account when using containers either by smart use of layers (placing everything in opaque scripts limits this), or externalizing caches by mounting them as volumes (which complicates configuration). Moreover it is desireable to run whole workflows locally, not just individual steps.
